...
//	private Statement  stmt;
	...
	public void addMember(MemberVO memberVO){
		try {
			//1. db연결
			//-> datasource 커넥션풀 공간에 저장되어 있는 db와 미리 연결을 맺은 connection 객체 얻기 
			...
			//2.DB에 INSERT회원가입 시킬 정보들을 MEMBERVO객체 내부의 각변수에 저장된 값 얻기 
				...
				//4. insert 문장 전체중에서..? 기호에 대응되는 값을 제외한 전체 문자열을  
				//oraclepreparedstatementwrapper 실행객체에 담아.. 이걸 리턴해줌
				//실행객체[ 자체를 반환 받는다.
				pstmt=con.prepareStatement(query);
				
				//5.?기호들에 대응되는 값들을 우리가 입력한 회원정보들로 설정해주자.
				...
				//6.db에 insert 
				pstmt.executeUpdate();
				//위의값을 이용해서 insert, update,delete 가능하다.
		} catch (Exception e) {
			System.out.println("addmember메소드 호출오류"+e);
		}finally {
			//자원해제 
			try {
				//8.자원해제
	...
	//memberform.html에서 입력한 회원정보들을 서블릿으로 전달해 
		//서블릿 내부에서 입력한 회원정보들을-> memberVO객체의 각변수에 저장하여 
		// memberDAO객체의 addmember메소드 호출시 매개변수로 전달하게 된다.
		//전달받은 memberVO 객체의 각변수값들을 이용해 db와 연결하여 insert작업하는 메소드
	...
	public void delMember(String id) {
		try {
			...
			//매개변수로 전달받은 회원 id에 해당되는 회원레코드를 삭제시켜라!
			String query="delete from t_member where id=?";
			
			
			//3. connection 객체의 preparestatement()메소드를 호출하여
			//dbms에 delete문장 전체를 전달하여 실행시킬 preparestatement실행객체를 반환받는다.
			//단 위 delete문장 전체를  preparestatement()호출시 전달하여 ?기호에 대응되는 설정값을 제외함
			
			pstmt=con.prepareStatement(query);
			
			//3-1 preparedstatment 실행객체에 ? 기호에 대응되는 값을 설정
			pstmt.setString(1,id);
			
			
			//4. preparedstatement실행객체의 excuteupdate메소드를 호출해
			//delete전체 문장을 db에 전송하여 실행!
			
			pstmt.executeUpdate();
			//insert, update, delete문장 실행가능 
			
		
		} 
		catch (Exception e) {
			System.out.println("delmember메소드 내부에서 오류 :"+e.toString());
	...
	//삭제 링크 a태그를 클릭했을떄 서블릿을 거텨 전송해온 회원 id를 delmember메소드의 인자로 전달받아
	//db에 저장되어있는 회원 한명의 정보를 삭제
